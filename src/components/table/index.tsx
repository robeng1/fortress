...
import Actionables, { ActionType } from "components/blocks/actionables"
...
>(({ className, children, ...props }, ref) => (
  <thead
    ref={ref}
    className={clsx(
      "whitespace-nowrap inter-sm-semibold text-grey-50 border-t border-b border-grey-20",
      className
    )}
    {...props}
  >
...
Table.Cell = React.forwardRef<HTMLTableCellElement, TableCellProps>(
  ({ className, linkTo, children, ...props }, ref) => (
    <td
      ref={ref}
      className={clsx("inter-sm-regular h-[40px]", className)}
      {...props}
      {...(linkTo && {
        onClick: (e) => {
          // navigate(linkTo)
...
Table.Row = React.forwardRef<HTMLTableRowElement, TableRowProps>(
  ({ className, actions, children, linkTo, forceDropdown, ...props }, ref) => (
    <tr
      className={clsx(
        "inter-sm-regular border-t border-b border-grey-20 text-grey-90",
        className,
...
export const TablePagination = ({
  ...
  return (
    <div
      className={clsx(
        "flex w-full justify-between inter-sm-regular text-grey-50 mt-14",
        className
      )}
    >
