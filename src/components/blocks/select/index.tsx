...
import InputHeader, { InputHeaderProps } from "components/blocks/input-header"
...
type MultiSelectProps = InputHeaderProps & {
  ...
  labelledBy?: string
  options: { label: string; key: string | null; disabled?: boolean }[]
  value: SelectOption[] | SelectOption | null
  filterOptions?: (term: string, callback) => Promise<any>
...
const ClearIndicator = ({ ...props }: ClearIndicatorProps<any>) => {
  ...
  return (
    <div
      onMouseDown={(e) => {
        if (restInnerProps?.onMouseDown) restInnerProps.onMouseDown(e)
      }}
      ref={ref}
...
const Option = ({ className, ...props }: OptionProps<any>) => {
  return (
    <components.Option
      {...props}
...
const GetSelect = React.forwardRef(
  (
    { isCreatable, searchBackend, onCreateOption, handleClose, ...props }: any,
    ref
  ) => {
    if (isCreatable) {
