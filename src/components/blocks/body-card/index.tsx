...
const BodyCard: React.FC<BodyCardProps> = ({
  ...
  return (
    ...
    >
      {isScrolled && (
        <div className="absolute rounded-t-rounded top-0 left-0 right-0 bg-gradient-to-b from-grey-0 to-[rgba(255,255,255,0)] h-xl z-10" />
      )}
      <div
        className="pt-md px-xl flex flex-col grow overflow-y-auto"
        onScroll={scrollListener}
      >
        <div className="flex items-center justify-between mt-6 h-xl">
          {customHeader ? (
            <div>{customHeader}</div>
          ) : title ? (
            <h1 className="inter-xl-semibold text-grey-90">{title}</h1>
          ) : (
            <div />
          )}
            ...
            {events.map((event, i: React.Key) => {
              return (
                <Button
                  ...
                  className="first:ml-xs min-w-[130px] justify-center"
