...
import { mToSFormatted, mToSFormattedK } from "utils/money"
      ...
      <div className="md:flex justify-between items-center space-y-4 md:space-y-0 space-x-2">
        ...
        <div className="flex items-start space-x-2 md:space-x-4">
          <div className="w-9 h-9 shrink-0 mt-1">
            <div className="w-8 h-8 relative flex justify-center items-center rounded-full bg-gray-500 text-xl text-white uppercase">
              {txn.txn_kind.charAt(0)}
            </div>
          </div>
          <div>
        ...
        <div className="flex items-center space-x-2 md:space-x-4 pl-3 md:pl-0">
          ...
          <div
            className={`text-right text-gray-800 ${
              txn.minor_amount < 0 ? "text-gray-800 " : "text-green-500"
            }`}
          >
            {txn.minor_amount < 0 ? "" : "+"}
            {mToSFormattedK({
